...
public class Server {
    ...
    public static Map<String, String> verifyCodes = new HashMap<>();//验证码map key=id, val=code
    public static Map<String, DataPack> registerTempData = new HashMap<>();//注册验证 key=id, val=data
    ...
    private static void handleClient(Socket clientSocket) {
        try (
            ...
            try {
                ...
                if(request.equals("login")) {
                ...
                else if(request.equals("verifymail")) {
                    Account api = new AccountSys();
                    //解析json字符串
                    String id = obj.getString("id");
                    String verify = obj.getString("verify");

                    response = api.verifyMail(id, verify);
                }
                ...
                else if(request.equals("sendverification")) {
                    Account api = new AccountSys();
                    //解析json字符串
                    String id = obj.getString("id");

                    response = api.sendVerificationCode(id);
                }
                else if(request.equals("retrievepwd")) {
                    Account api = new AccountSys();
                    //解析json字符串
                    String id = obj.getString("id");
                    String verify = obj.getString("verify");
                    ...
                    response = api.retPwd(id, verify, newPwd);
                }
