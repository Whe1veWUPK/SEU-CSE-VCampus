...
public class CourseDelDlg extends JFrame {
    ...
    public CourseDelDlg(String userId) {
        _clientID = userId;
        init();
    }

    void init() {
        JFrame InfoInput = new JFrame("学生退课");
        ...
        // 标签和文本框
        JLabel labelStuNum = new JLabel("学号：");
        labelStuNum.setFont(new Font("华文中宋", Font.PLAIN, 17));
        labelStuNum.setBounds(50, 100, 90, 30);
        InfoInput.add(labelStuNum);
        JTextField textStuNum = new JTextField();
        textStuNum.setBounds(140, 100, 180, 30);
        InfoInput.add(textStuNum);

        // 设置底下按钮
        JButton btnControl = new JButton("退课");
        ...
        btnControl.addActionListener(e -> {
            try {
                jsonObject.put("request", "delstufromcour");
                jsonObject.put("usrid", _clientID);
                jsonObject.put("courid", textCourId.getText());
                jsonObject.put("stunum", textStuNum.getText());
                ...
                switch (res.getString("code")) {
                    ...
                    case "4":
                        ...
                        break;
                    case "5":
                        JOptionPane.showMessageDialog(this, "该学生已选过这门课程", "错误", JOptionPane.ERROR_MESSAGE);
                        break;
                    case "-1":
